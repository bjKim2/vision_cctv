...
class ImageViewer(QMainWindow):
    def __init__(self,imgs,save_dir):
        ...
        self.openImage(image=self.toQImage(cv2.resize(image,(3000,1500))))
    ...
    def keyPressEvent(self, e):
        if e.key() == 65:
            ...
            try:
                shutil.move(self.img_list[self.pos] ,os.path.join(self.save_dir + '/images') +'/' + os.path.basename(self.img_list[self.pos] ))
                shutil.move( self.img_list[self.pos].replace('images','labels')[:-3] + 'txt' ,os.path.join(self.save_dir + '/labels') + '/' +os.path.basename(self.label_list[self.pos]) )
                ...
                self.openImage(image=self.toQImage((cv2.putText(img=image , text='moved',color=(255,0,0),org=(int(self.width/2.3),self.height//4), fontScale=1,thickness=4,fontFace=cv2.FONT_HERSHEY_COMPLEX))))
            except:
