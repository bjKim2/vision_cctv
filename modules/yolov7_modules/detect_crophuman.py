...
def detect(save_img=False):
    ...
    for path, img, im0s, vid_cap in dataset:
        ...
        with open (path.replace('images','labels')[:-3] + 'txt', 'r' ) as f:
            boxes = f.readlines() 

        boxes= [xywh2clsxyxy(box.strip().split(' '),*img_size) for box in boxes]
        # print(boxes[0])
                    ...
                    if save_img or view_img:  # Add bbox to image
                        ...
                        if int(cls) == 0:
                            ...
                            if len(re_box):
                                ...
                                with open(os.path.dirname(save_path) + '/labels/' + os.path.basename(save_path)[:-4] + '_' + str(count) + '.txt' ,'w') as f:
                                    for num3, box in enumerate(re_box):
                                        # print('box, xyxy : ',box,xyxy)
                                        for i,cord5 in enumerate(box):
                                            if i ==0:
                                                continue
                                            if i % 2 == 1:
                                                # print('box[i],int(xyxy[i-1]):',box[i],int(xyxy[i-1]))
                                                if box[i] < int(xyxy[0]) :
                                                    # print('box[i]',box[i])
                                                    box[i] = int(xyxy[0])
                                                    # print('box[i]',box[i])
                                                elif box[i] > int(xyxy[2]):
                                                    box[i] = int(xyxy[2])
                                                box[i] -= int(xyxy[0])
                                                # print('box[i]',box[i])
                                            else:
                                                if box[i] < int(xyxy[1]) :
                                                    box[i] = int(xyxy[1])
                                                elif box[i] > int(xyxy[3]):
                                                    box[i] = int(xyxy[3])
                                                box[i] -= int(xyxy[1])
                                        # print(box)
                                        box = box[0:1] + cvt_yoloformat(box[1:],w= int(xyxy[2]) - int(xyxy[0]), h= int(xyxy[3]) - int(xyxy[1]))
            
                                        for num4,coord4 in enumerate(box):
                                            if num4 != len(box)-1:
                                                f.write(str(coord4) + ' ')
                                            else:
                                                f.write(str(coord4) + '\n')
