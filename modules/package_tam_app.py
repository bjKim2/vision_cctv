...
def vos_tracking_video(video_state, interactive_state, mask_dropdown):
    ...
    if True:
        ...
        if not os.path.exists('./result/mask/{}/yolo'.format(video_state["video_name"].split('.')[0])):
            os.makedirs('./result/mask/{}/yolo'.format(video_state["video_name"].split('.')[0]))            
        ...
        for mask in video_state["masks"]:
            # mask .npy
            print(i)
            with open(os.path.join('./result/mask/{}/yolo'.format(video_state["video_name"].split('.')[0]), '{:05d}.txt'.format(i)) , 'w') as f:
                w , h = mask.shape[1], mask.shape[0]
                cls_num = list(np.unique(mask)[1:])
                for j in cls_num:    
                    x1 = np.unique(np.where(mask == j)[1]).min()
                    x2 = np.unique(np.where(mask == j)[1]).max()
                    y1 = np.unique(np.where(mask == j)[0]).min()
                    y2 = np.unique(np.where(mask == j)[0]).max()
                    xywh = cvt_yoloformat([x1,y1,x2,y2], w, h)
                    line = ' '.join(map(str,[j-1] + xywh)) + ' \n'
                    f.write(line)
